...
function renderImages(urls) {
  ...
  const images = urls.map(u => {
    ...
    return `<a class="item-image-link" href="#" data-full-url="${safeUrl}"><img class="item-image" src="${safeUrl}" alt="Item attachment"></a>`;
  }).join('');
...
(async () => {
  ...
  // Delegate image clicks
  document.addEventListener('click', (e) => {
    const link = e.target.closest('.item-image-link');
    if (link) {
      e.preventDefault();
      const url = link.getAttribute('data-full-url');
